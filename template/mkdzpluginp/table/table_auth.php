 ...
 * 用户权限表
 **/
class table__FILENAME__auth extends discuz_table
{
    public function __construct() {
		$this->_table = '_FILENAME__auth';
		$this->_pk = 'uid';
		parent::__construct();
	}
    ...
    public function set()
    {
        $uid = _FILENAME__validate::getNCParameter('uid','uid','integer');
        $auth = _FILENAME__validate::getNCParameter('auth','auth','integer');
        $ctime = date("Y-m-d H:i:s");
        $sql = "INSERT INTO ".DB::table('_FILENAME__auth')." (uid,auth,ctime) VALUES ('$uid','$auth','$ctime') ".
	...
	public function query()
	{
        ...
        $auth  = _FILENAME__validate::getNCParameter('auth','auth','integer');
        $key   = _FILENAME__validate::getNCParameter('key','key','string',128);
        $sort  = _FILENAME__validate::getOPParameter('sort','sort','string',1024,'uid');
...
EOF;
        ...
        foreach ($return["root"] as &$row) {
            $row['groupname'] = C::m('#_FILENAME_#_FILENAME__usergroup')->grouptitle($row['groupid']);
