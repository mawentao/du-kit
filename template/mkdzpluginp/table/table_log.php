 ...
 * 插件访问日志表
 **/
class table__FILENAME__log extends discuz_table
{
	public function __construct() {
		$this->_table = '_FILENAME__log';
		$this->_pk = 'logid';
		parent::__construct();
	}
	...
	{/*{{{*/
		...
		$sday  = _FILENAME__validate::getNCParameter('sday','sday','string'); 
		$eday  = _FILENAME__validate::getNCParameter('eday','eday','string'); 
		$key   = _FILENAME__validate::getNCParameter('key','key','string'); 
        $sort  = _FILENAME__validate::getOPParameter('sort','sort','string',1024,'logtime');
        $dir   = _FILENAME__validate::getOPParameter('dir','dir','string',1024,'DESC');
		...
		$table__FILENAME__log = DB::table($this->_table);
		$table_common_member = DB::table('common_member');
		$sql = <<<EOF
SELECT SQL_CALC_FOUND_ROWS a.*,b.username,b.email
FROM $table__FILENAME__log as a LEFT JOIN $table_common_member as b ON a.uid=b.uid
...
FROM $table__FILENAME__log WHERE $where GROUP BY vday ORDER BY logtime ASC
EOF;
