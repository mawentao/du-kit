...
class HessianProxy extends FilterContainer{
    ...
    //可以通过options["encode"] 和 options["version"]来设置
  var $encode = 'GBK';
  var $version = PHP_MC_PACK_V2;
    ...
    if (is_array($options) && isset($options["encode"])) {
      $this->encode = $options["encode"];
    }
    if ($this->version != PHP_MC_PACK_V2) {
        $this->http->addHeader('Content-type',
            'application/baidu.mcpacklegacy-rpc;application/binary;charset='.$this->encode);
    } else {
        $this->http->addHeader('Content-type',
            'application/baidu.mcpack-rpc;application/binary;charset='.$this->encode);
    }
     ...
     *  确定本次RPC的字符编码
     *
     *  @param string ec 字符编码, 与http头中的编码类型保持一致
     **/
    function setEncode($ec = 'GBK') {
        $this->$encode = $ec;
        if ($this->version != PHP_MC_PACK_V2) {
            $this->http->addHeader('Content-type',
                'application/baidu.mcpacklegacy-rpc;application/binary;charset='.$this->encode);
    ...
    /**
     *  设置发送的Mcpack包类型，默认是Mcpack2
     *  可设置PHP_MC_PACK_V2 或者 PHP_MC_PACK_V1
     *
     *  @param PHP_MC_PACK_V2 或者 PHP_MC_PACK_V1
     **/
    
    function setMcpackVersion($version) {
    ...
    function getMcpackVersion($version) {
        return $this->version;
    }
	...
	function call($method,$params){
        ...
        //id 用随机数, 自动生成id
        $id = rand();
        $this->callingContext = array(JSONRPC_HEAD=>JSONRPC_VERSION, 'method'=>$method,'params'=>$params,'id' => "$id");
		if(empty($this->filters))
			return $this->executeCall($method,$params, $id);

		$wrapper = &new ProxyFilter($method,$params, $id);
	 ...
	 * @access public
	 **/
	function executeCall($method,$params,$id){
    ...
    //组装需要发送的数据
		$data = &$this->writer->writeCall($method,
        $params,
        "$id", 
        $this->version);
