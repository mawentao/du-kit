...
class ServiceInfoPHP4{
	 ...
	 * @access protected 
	 **/
	function &callMethod($method,&$params,&$writer, $id){
		if(!$this->isMethodCallable($method)){
			$writer->setFault('1',"Method $method does not exist in this service",$id);
			return null;
		}
...
class HessianService{
	...
	function service(){
		...
		// uso de streams para obtener informaci髇 cruda del post
        ...
        //json rpc 协议 {"jsonrpc": "2.0", "method": ..., "params": ..., "id": ...}
        //解析错误在这里判断
        $arr = &$this->parser->parseCall();
        $method = &$arr['method'];
        //返回的id, json rpc 规定的
        $id = $arr['id'];
		$result = null;
		...
		if(Hessian::isError($method)){
            ...
            $this->writer->setFault($method->code,
                        $method->message,
                        "$id",
                        $method->getError()
                    );
            ...
            $params = &$arr['params'];
            //函数调用
			$result = $this->serviceInfo->callMethod($method,$params,$this->writer,$id);
            $reply_version = mc_pack_get_version($postData);
        }

        //返回的rpc {"jsonrpc": "2.0", "result": 19, "id": 3}
        //失败了在callMethod里面会设置Fault
        $reply = $this->writer->writeReply(
            array(JSONRPC_HEAD=>JSONRPC_VERSION, //版本号
                'result'=>$result,         //运行结果
                'id' => "$id"               //id号
            ),
            $reply_version
        ...
        if ($reply_version != PHP_MC_PACK_V2) {
            header('Content-type:application/baidu.mcpacklegacy-rpc;application/binary;charset='.$this->encode);
        } else {
            header('Content-type:application/baidu.mcpack-rpc;application/binary;charset='.$this->encode);
        }
