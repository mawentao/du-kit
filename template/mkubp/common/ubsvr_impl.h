 ...
 * $Id: ubsvr_impl.h, v1.0.0 __DATETIME__, Exp $
 *
 **************************************************************************/
 ...
 * @file   ubsvr_impl.h
 * @date   __DATETIME__
 * @brief  ub_server框架具体实现
 **/

#ifndef ___UBSVR_IMPL_H__
#define ___UBSVR_IMPL_H__
...
namespace _NAMESPACE_
{
    ...
    {/*{{{*/
        ...
        for(uint32_t ithrd = 0; ithrd < _ub_svr_conf.thread_num; ithrd++)
        {
            ...
            if(shared_init && _NAMESPACE_::SUCC != shared_init(ithrd, shared_buf->user_data))
            {
    ...
    int Server<_IdlReq, _IdlRes, _IdlPool>::_load_idl_req(
                                            ...
                                            idl_req_t &req)
    {/*{{{*/
        check_null(mcpack_buf, _NAMESPACE_::FAIL);
        check_null(shared_data, _NAMESPACE_::FAIL);

        int func_ret = _NAMESPACE_::SUCC;
        try
        {
            // 打开mcpack
    ...
    {/*{{{*/
        ...
        while(retry_save && _NAMESPACE_::SUCC == func_ret)
        {
            retry_save = false;
    ...
    {/*{{{*/
        ...
        check_null(ph_req, _NAMESPACE_::FAIL);
        check_null(ph_res, _NAMESPACE_::FAIL);
        check_null(shared_data, _NAMESPACE_::FAIL);

#if 0
        //! ip authenticate
        if(_NAMESPACE_::SUCC != shared_data->svr_handle->_ip_auth.auth_ip_fd(ub_server_get_socket()))
        {
        ...
        int session_ret = _NAMESPACE_::SUCC;
		//! 处理、响应请求
        try
        {
            ...
            if(_NAMESPACE_::SUCC != _load_idl_req((char*)(ph_req+1), ph_req->body_len, shared_data, req))
            {
                throw (bsl::Exception() << BSL_EARG)
            ...
            if(_NAMESPACE_::SUCC != session_ret)
            {
                throw (bsl::Exception() << BSL_EARG)
            ...
            // save idl to mcpack
            if(_NAMESPACE_::SUCC != _save_idl_res(res, shared_data, (char*)(ph_res+1), *ph_res))
            {
                throw (bsl::Exception() << BSL_EARG)
